...
# Author of final code: Chelsea Rowe 1412535
...
def remove_common_words(book_words, common_words):
    words_minus_common_list = []
    for word in book_words:
        if word not in common_words:
            words_minus_common_list.append(word)
    return words_minus_common_list
...
def count_number_of_words_that_appear_in_another_list(text_words, other_list):
    ...
    for word in text_words:
        if word in other_list:
            match_count += 1
    return match_count  # Need to implement this function
...
def average_word_length(text_words):
    count_avg_length = 0
    for word in text_words:
        count_avg_length += len(word)
    final_avg = (count_avg_length / len(text_words))
    return final_avg  # Need to implement this function
...
def calculate_word_frequency(text_words):
    ...
    for word in text_words:
        if word in word_freq:
            ...
            word_freq[word] = 1
    return word_freq  # {"need to implement": "calculate_word_frequency"}
...
def calculate_sentiment(word_list, positive_words, negative_words):
    ...
    positive_percentage = float(positive_count / len(word_list)) * 100
    negative_percentage = float(negative_count / len(word_list)) * 100
    sentiment_difference = positive_percentage - negative_percentage
    return round(sentiment_difference,
                 2)  # Implement and replace 0.0 with the difference between the positive and negative percents.
